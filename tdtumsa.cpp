...
void FinalPage::initializePage()
{
    ...
    strCodeMux.append("tscbrmuxer b:15040 pat.ts b:3008 sdt.ts b:3008 nit.ts b:7520 tot.ts b:3008 eit.ts ");
    bitratetotal=0;
    for(int it=0;it<(nservice+oneseg-1);it++){
        auxString1=QVariant(it).toString();
        auxString2=QVariant(it-oneseg+1).toString();
        if(oneseg==1&&it==0){
            strCodeMux.append("b:7520 pmt"+field("PID PMT"+auxString2).toString()+".ts ");
            bitratetotal+=field("VideoBitRate"+auxString2).toInt()*(1000);                
